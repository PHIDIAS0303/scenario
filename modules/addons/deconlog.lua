...
if config.decon_area then
	Event.add(defines.events.on_player_deconstructed_area, function(e)
		...
		if #items > 250 then
			print_to_players(true, player.name .. ' tried to deconstruct the area ' .. pos_to_gps_string(e.area.left_top) .. ' to ' .. pos_to_gps_string(e.area.right_bottom) .. 'on surface' .. player.surface .. 'that have ' .. format_number(#items) .. ' items, but were not allowed.')
		end

		add_log(get_secs() .. ',' .. player.name .. ',decon_area,' .. pos_to_string(e.area.left_top) .. ',' .. pos_to_string(e.area.right_bottom) .. '(' .. player.surface .. ')')
	end)
end
